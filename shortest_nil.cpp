...
public:
  ...
  void insert(subset& obj, unsigned nobits) {
    ...
    if(no_nodes % 100000 == 0) {
      cout << setprecision(7) << omp_get_wtime() - qt  << "\tqptr: " << current_node << "\tqsize: " << no_nodes << " - " << " nxt-dst: " << next_distance << " - " << " minbitcnt " << min_bit_count 
	   << " - eq (f-t): " << (double)(existsFalse) / (existsFalse + existsTrue) << " - " << (double)(existsTrue) / (existsFalse + existsTrue) 
	   << " - ss (f-t): " << (double)(supersetFalse) / (supersetFalse + supersetTrue) << " - " << (double)(supersetTrue) / (supersetFalse + supersetTrue) 
	   << " - (" << existsFalse << " " << existsTrue << " " << supersetFalse << " " << supersetTrue << ")" 
	   << "\tbc-2: " << bit_counts[2] << " bc-3: " << bit_counts[3] << " bc-4: " <<  bit_counts[4] << endl;
      qt = omp_get_wtime();
...
int main(int argc, char** argv) {
  if (argc != 6) {
    ...
    //Engincan File_Out
  ofstream out;
  string randomfileName = "Automata.txt";
  out.open(randomfileName);

  unsigned long long lastForwardNode;

  //for(int randomIndex=0; randomIndex<10; randomIndex++)
  //{
    //N = rand() % (atoi(argv[1])-80+1) + 80 ;
...
*/ 
  ...
  ofstream myFile;
  string randomMyFileName = "stateSequences.txt";
  myFile.open(randomMyFileName);
  vector<vector<int>>subsequences;
  int count = 0; //added -n
  int beginning=0; //added -n
  ...
  for (auto letter: syncSequence)
  {
	  ...
	  myFile << "(" << currentSet->no_bits() << ") : ";
	  for(int k = 0; k < N; k++)
		if(currentSet->get_bit(k))
			myFile << k << " ";
	  myFile<< endl;
	  ...
	  //added the block down -n
	 if(currentSet->no_bits() > nextSet->no_bits() ){
		 vector<int> subseq;
		  cout << "sequence grouped: "; 
		  myFile << "the sequence necessary to go from " << currentSet->no_bits() <<" states to "  << nextSet->no_bits() << " states: ";
		  for (int j=beginning; j<count; j++){
			 cout << syncSequence[j] ;
			 myFile << syncSequence[j] ;
			 subseq.push_back(syncSequence[j]);
		  }
		  beginning=count;
  ...
  //Engincan File_Out
  printAutomataToFile(automata,N,P,out);
  out << "Shorest Sequence: ";
  for (auto letter: syncSequence)
      out << letter;
  ...
  //cout << "\n\nRandomIndex " << randomIndex << " is done." << endl;}
