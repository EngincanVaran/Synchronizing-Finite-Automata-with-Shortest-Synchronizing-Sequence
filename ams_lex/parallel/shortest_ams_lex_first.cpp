...
bool checkSubset(AMSLex::CandidateList &haystack, AMSLex::CandidateList &needles, uint32_t num_threads) {
  ...
  #pragma omp parallel for firstprivate(flag_prv) num_threads(num_threads)
   for (uint32_t i = 0; i < needles.size(); ++i) {
     if (flag) continue;
     for (uint32_t j = 0; j < haystack.size() && !flag_prv; ++j) {
       if (haystack[j]->size() < needles[i]->size()) continue;
       flag_prv = 1;
       for (auto itr_n = needles[i]->begin(); itr_n != needles[i]->end() && flag_prv; ++itr_n) {
         flag_prv = 0;
         for (auto itr_h = haystack[j]->begin(); itr_h != haystack[j]->end() && !flag_prv; ++itr_h)
           if (*itr_h == *itr_n) flag_prv = 1;
           else if (*itr_h > *itr_n) break;
       }
     }
     if (flag_prv) {
       #pragma omp atomic
...
int main(int argc, char **argv) {
  ...
  AMSLex ap(obj_count, num_threads);
  ...
  sort(iBEl.begin(), iBEl.end(), AMSLex::candidate_compare);
...
//exit(1);
  ...
  while (current != nullptr) {
    ...
    if (frontier_starts == current_node) {
      ...
      no_nodes -= (size - fs); bEl.resize(no_nodes); double hTime = omp_get_wtime();
      if (checkSubset(iBEl, bElF, num_threads)) { //if anything subsumed
